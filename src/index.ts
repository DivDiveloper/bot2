export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // 🍪 מחרוזת העוגיות החדשה והתקפה שסיפקת כעת
    ...
    // 📡 נשתמש במתודת החיפוש המעובדת כדי למנוע את הגנת ה-Photosets שחוסמת החזרת HTML
    const buildFlickrUrl = (key: string) => {
      return "https://flickr.com" +
             "?method=flickr.photos.search" + 
             "&api_key=" + key +
             "&user_id=" + targetUser +
             "&safe_search=3" +                      // דרישת S3
             "&per_page=10" +                        // שליפת 10 תמונות
             "&extras=safety_level" +                // פירט רמת ה-Safety ב-JSON
             "&format=json" +
             "&nojsoncallback=1" +
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Primary key returned HTML instead of JSON. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "search_primary_fresh", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ error: "Server redirected to homepage on both keys.", server_response: errorText.substring(0, 200) }),
            { status: 502, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
          );
        }

        const data = await fallbackResponse.json();
        return new Response(JSON.stringify({ source: "search_backup_fresh", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal Route Exception", primary: primaryError.message, backup: backupError.message }),
