export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // ✅ שילוב מנצח: נתיב ה-ajax הפנימי יחד עם מתודת החיפוש הכללית
    const buildFlickrUrl = (key: string) => {
      return "https://flickr.com" +
             "?method=flickr.photos.search" + 
             "&api_key=" + key +
             "&user_id=" + targetUser +
             "&safe_search=3" +                      // בקשת S3
             "&per_page=10" +                        // 10 תמונות
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Primary AJAX search failed. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "ajax_search_primary_s3", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ error: "Flickr blocked AJAX access completely.", server_response: errorText.substring(0, 200) }),
            { status: 502, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
          );
        }

        const data = await fallbackResponse.json();
        return new Response(JSON.stringify({ source: "ajax_search_backup_s3", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal AJAX Exception", primary: primaryError.message, backup: backupError.message }),
