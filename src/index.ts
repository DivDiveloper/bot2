export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // ✅ בנייה מאובטחת באמצעות אובייקט URL מובנה כדי למנוע השמטת נתיבים
    const buildFlickrUrl = (key: string) => {
      ...
      url.searchParams.set("safe_search", "3");
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Primary internal API route failed. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "url_object_primary_success", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ 
              error: "Both keys failed with URL Object architecture.", 
        ...
        return new Response(JSON.stringify({ source: "url_object_backup_success", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
