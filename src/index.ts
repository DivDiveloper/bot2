export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // 🍪 מחרוזת העוגיות המלאה שחילצת מהאנדרואיד
    ...
    // 🌐 נקודת הקצה לביצוע POST
    const flickrUrl = "https://flickr.com";

    // פונקציית עזר לבניית גוף הבקשה (Form Data) - חובה להעביר את כל הפרמטרים כאן ב-POST
    const createBody = (key: string) => {
      const formData = new URLSearchParams();
      formData.append("method", "flickr.photosets.getPhotos");
      formData.append("api_key", key);
      formData.append("user_id", targetUser);
      formData.append("photoset_id", photosetId);
      formData.append("privacy_filter", "1");
      formData.append("per_page", "10");
      formData.append("extras", "safety_level");
      formData.append("format", "json");
      formData.append("nojsoncallback", "1");
      formData.append("csrf", csrfToken); // ה-CSRF מוזרק ישירות לתוך גוף הנתונים
      return formData.toString();
    };

    const commonHeaders = {
      "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
      "Content-Type": "application/x-www-form-urlencoded", // חובה ב-POST כדי שהשרת יקרא את ה-Body
    ...
    try {
      const response = await fetch(flickrUrl, {
        method: "POST", // שינוי ל-POST מלא תקין
        headers: commonHeaders,
        body: createBody(primaryApiKey) // הפרמטרים נשלחים כאן ולא ב-URL
      });

      const contentType = response.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) {
        throw new Error("Primary API key failed or returned HTML. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "primary_key_post", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ 
              error: "Both keys failed on POST request.", 
        ...
        return new Response(JSON.stringify({ source: "backup_key_post", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal POST Route Exception", primary: primaryError.message, backup: backupError.message }),
