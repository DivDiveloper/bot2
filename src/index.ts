export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // 🍪 מחרוזת העוגיות העדכנית והתקפה שלך
    ...
    // 📡 שימוש ב-GET לנתיב ה-ajax הפנימי, ללא פרמטר ה-csrf ב-URL (האימות מבוצע קשיח דרך ה-Cookie)
    const buildFlickrUrl = (key: string) => {
      return "https://flickr.com" +
             "?method=flickr.photos.search" + 
             "&api_key=" + key +
             "&user_id=" + targetUser +
             "&safe_search=3" +                      // דרישת S3 לרמת הסינון
             "&per_page=10" +                        // בקשה ל-10 תמונות
             "&extras=safety_level" +                // חשיפת רמת ה-S ב-JSON
             "&format=json" +
             "&nojsoncallback=1";                    // מונע את ה-Redirect ועוטף כ-JSON נקי
    };

    const commonHeaders = {
      // 🔑 שינוי קריטי: הגדרת User-Agent של דפדפן אנדרואיד מובייל עדכני כדי לעקוף את חסימת השרת
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Primary internal route returned HTML. Trying backup key.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "ajax_get_primary_s3_fixed", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ 
              error: "Flickr internal router denied access completely.", 
              server_response: errorText.substring(0, 200),
              called_url: fallbackUrl
            }),
        ...
        return new Response(JSON.stringify({ source: "ajax_get_backup_s3_fixed", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal Fixed Route Exception", primary: primaryError.message, backup: backupError.message }),
