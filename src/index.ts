export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // ✅ בניית מחרוזת קשיחה לחלוטין ללא שימוש באובייקט URL כדי למנוע השמטת נתיבי ה-ajax
    const buildStrictUrl = (key: string) => {
    ...
    // --- ניסיון 1: מפתח ראשי ---
    try {
      const url = buildStrictUrl(primaryApiKey);
      const response = await fetch(url, { 
        method: "GET", 
        headers: commonHeaders,
        redirect: "manual" // 🔑 מונע מהוורקר לעקוב אחרי הניתוב הויזואלי לעמוד הבית ומכריח לעבד את התשובה המקורית
      });

      const contentType = response.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) {
        throw new Error("Primary hardcoded route bypassed JSON. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "strict_primary_s3", data }, null, 2), {
        status: 200,
      ...
      // --- ניסיון 2: מפתח גיבוי ---
      try {
        const fallbackUrl = buildStrictUrl(backupApiKey);
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ 
              error: "Strict routing blocked on both keys.", 
        ...
        return new Response(JSON.stringify({ source: "strict_backup_s3", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal Strict Exception", primary: primaryError.message, backup: backupError.message }),
