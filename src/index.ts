export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // ✅ שימוש בדומיין ה-API הרשמי שלא מאפשר הפניות לעמודי HTML ויזואליים
    const buildApiUrl = (key: string) => {
    ...
    try {
      const url = buildApiUrl(primaryApiKey);
      const response = await fetch(url, { method: "GET", headers: commonHeaders });
      const contentType = response.headers.get("content-type") || "";

      if (!contentType.includes("application/json")) {
        throw new Error("Primary API endpoint failed. Trying backup.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "api_subdomain_primary", data }, null, 2), {
        status: 200,
      ...
      try {
        const fallbackUrl = buildApiUrl(backupApiKey);
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ 
              error: "Flickr api subdomain rejected request.", 
        ...
        return new Response(JSON.stringify({ source: "api_subdomain_backup", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal API Subdomain Exception", primary: primaryError.message, backup: backupError.message }),
