export default {
  async fetch(request: Request): Promise<Response> {
    ...
    // 📡 חזרה למתודת החיפוש שעבדה, עם נתיב ה-API הנכון
    const buildFlickrUrl = (key: string) => {
      return "https://flickr.com" +
             "?method=flickr.photos.search" + 
             "&api_key=" + key +
             "&user_id=" + targetUser +
             "&privacy_filter=1" +                   // פותח גישה לרמות S3 עבור משתמש מחובר
    ...
    try {
      ...
      if (!contentType.includes("application/json")) {
        throw new Error("Primary search failed. Trying backup.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "search_primary_s3", data }, null, 2), {
        status: 200,
      ...
      try {
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ error: "Search failed on both keys.", server_response: errorText.substring(0, 200) }),
            { status: 502, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
          );
        }

        const data = await fallbackResponse.json();
        return new Response(JSON.stringify({ source: "search_backup_s3", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal Search Exception", primary: primaryError.message, backup: backupError.message }),
