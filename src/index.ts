...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // 🛡️ הגנה מפני בוטים: אם הבקשה מגיעה מסורק או בוט (כמו Google-Lens), נחסום אותה מייד
    if (
      userAgent.includes("Bot") || 
      userAgent.includes("Spider") || 
      userAgent.includes("Lens") || 
      userAgent.includes("crawler")
    ) {
      return new Response("Bot requests are not allowed to trigger the browser.", { status: 403 });
    }

    const targetUser = "59871731@N06"; 
    
    // 1. משיכת הקוקיז מתוך מאגר ה-STORE שלך
    let userCookieString = await env.STORE.get("cookies");
    
    // גיבוי אוטומטי אם ה-KV עדיין ריק
    if (!userCookieString) {
    ...
    // 2. הפעלת הדפדפן הייעודי של קלאודפלייר
    const browser = await puppeteer.launch(env.MYBROWSER);
    const page = await browser.newPage();

    try {
      // 3. פירוק קשיח ומאובטח של מחרוזת ה-Cookie כדי למנוע קריסת דפדפן
