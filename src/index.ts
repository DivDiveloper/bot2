export default {
  async fetch(request: Request): Promise<Response> {
    ...
    const commonHeaders = {
      ...
      "X-Requested-With": "XMLHttpRequest", // 🔑 כותרת קריטית לאימות קריאות AJAX פנימיות
    ...
    try {
      const response = await fetch(flickrUrl, {
        method: "POST", // שינוי ל-POST כדי למנוע את הניתוב ה-Visual
        headers: commonHeaders,
        body: createBody(primaryApiKey)
      });

      const contentType = response.headers.get("content-type") || "";
      if (!contentType.includes("application/json")) {
        throw new Error("Primary AJAX POST failed. Trying fallback.");
      }

      const data = await response.json();
      return new Response(JSON.stringify({ source: "ajax_post_primary_s3", data }, null, 2), {
        status: 200,
      ...
      // --- ניסיון 2: מפתח גיבוי ---
      try {
        const fallbackResponse = await fetch(flickrUrl, {
          method: "POST",
          headers: commonHeaders,
          body: createBody(backupApiKey)
        });
        ...
        if (!fallbackContentType.includes("application/json")) {
          ...
          return new Response(
            JSON.stringify({ error: "Flickr gateway completely blocked POST access.", server_response: errorText.substring(0, 200) }),
            { status: 502, headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }
          );
        }

        const data = await fallbackResponse.json();
        return new Response(JSON.stringify({ source: "ajax_post_backup_s3", data }, null, 2), {
          status: 200,
          headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" }
        });
        ...
        return new Response(
          JSON.stringify({ error: "Fatal POST Ajax Exception", primary: primaryError.message, backup: backupError.message }),
